...
export function DragAndDrop(props: MainInterface) {
  ...
  useEffect(() => {
    const updateFieldLine = async (state: FieldClass[]) => {
      const res = await props.comp.updateFieldLine(state, props.pickComp)
      console.log(res)
    }

    if (JSON.stringify(state) !== JSON.stringify(props.comp.fields_ids)) {
      const res = updateFieldLine(state)
      console.log(res)
      if (!res) handlers.setState(props.comp.fields_ids)
    }
  }, [state])
