...
interface UpdateStringValueField extends MainInterface {
  field: FieldClass;
  dataName: StringKeys<FieldClass>;
  func: any;
...
export function UpdateStringValueField(props: UpdateStringValueField) {
  ...
  const defaultItem = () => {
    return (
      <Group gap={7} align="flex-start">
        ...
        <Text>{props.field[props.dataName] as unknown as string}</Text>
      </Group>
    )
  }
  const updatedtItem = () => {
  ...
  const editedtItem = () => {
    return (
      ...
      <Group gap={7} align="center">
        ...
        <Tooltip fz="xs" label={valid() ? props.text?.save : props.text?.empty + ' / ' + props.text?.badsimvols} position="top" transitionProps={{ duration: 0 }}>
          ...
          onClick={async () => {
            if(valid()) {
              ...
              const res = await props.func(props.field._id, props.dataName, value, props.comp, props.pickComp)
              if(!res) {
        ...
        value={value}
        variant="unstyled"
          placeholder={props.field[props.dataName] as unknown as string}
          autoFocus
        />
      </Group>
      {examples()}
