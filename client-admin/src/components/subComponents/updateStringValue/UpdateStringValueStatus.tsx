...
interface UpdateStringValueStatus extends MainInterface {
  status: StatusClass;
  dataName: StringKeys<StatusClass>;
  func: any;
...
export function UpdateStringValueStatus(props: UpdateStringValueStatus) {
  ...
  const defaultItem = () => {
    return (
      <Group gap={7} align="flex-start">
        ...
        <Text>{props.status[props.dataName] as unknown as string}</Text>
      </Group>
    )
  }
  const updatedtItem = () => {
  ...
  const editedtItem = () => {
    return (
      ...
      <Group gap={7} align="center">
        ...
        <Tooltip fz="xs" label={valid() ? props.text?.save : props.text?.empty + ' / ' + props.text?.badsimvols} position="top" transitionProps={{ duration: 0 }}>
          ...
          onClick={async () => {
            if(valid()) {
              ...
              const res = await props.func(props.status._id, props.dataName, value, props.comp, props.pickComp)
              if(!res) {
              ...
              props.setSelectedStatus(new StatusClass(res))
              setStep(1)
            }
          }}
        />
        ...
        value={value}
        variant="unstyled"
          placeholder={props.status[props.dataName] as unknown as string}
          autoFocus
        />
      </Group>
      {examples()}
